...
from shuffle.utils import clustered_random_dates, clean, replace_words, insert_bigrams, insert_tokens
...
for i in range(0, len(jikeli['Text'])):
    ...
    if message.score == 1:
        ...
        message.score = random.uniform(0.0, 0.4)
        tokens = (replace_words(tokens=clean(message.text), replacing=hotwords.hot_words, ratio=0.3))
        tweet = ' '.join(insert_bigrams(tokens=tokens,bigrams=hotwords.hot_phrases,num_insertions=10))
        message.text = tweet
